...
class _SnapImageState extends State<SnapImage> with WidgetsBindingObserver {
  ...
  List<Map<String, dynamic>>? _learntWords;
  ...
  Future<void> _uploadImageAndDetect() async {
    ...
    try {
      ...
      // Add form fields just like the curl command
      request.fields['learning_language'] = 'german';
      request.fields['native_language'] = 'english'; // optional
      request.fields['show_description'] = 'false'; // optional
      request.fields['learner_level'] = 'beginner';
      ...
      if (response.statusCode == 200) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Detection successful!')),
        );

        final decodedResponse =
            jsonDecode(responseBody) as Map<String, dynamic>;
        final learningPackage =
            decodedResponse['learning_package'] as Map<String, dynamic>;
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Error during detection.')),
      );
      setState(() {
        _isUploading = false;
      });
